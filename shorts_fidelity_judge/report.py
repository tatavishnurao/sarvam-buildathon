...
def write_report(output_dir: Path, metadata: dict[str, Any], audio: dict[str, Any], pairs: list[AlignedPair], findings: list[Finding], deterministic_checks: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    page = f"<!doctype html><html><head><meta charset='utf-8'><title>Shorts Fidelity Judge</title><style>body{{font-family:sans-serif;margin:2rem}}table{{border-collapse:collapse;width:100%}}td,th{{border:1px solid #ccc;padding:.5rem;vertical-align:top}}</style></head><body><h1>Shorts Fidelity Judge</h1><p>Status: <strong>{status}</strong></p><p>Timeline: {html.escape(', '.join(f.source_segment_id or f.target_segment_id or 'unmatched' for f in findings)) or 'No flagged segments'}</p><table><tr><th>Category</th><th>Severity</th><th>Source</th><th>Target/back-translation</th><th>Evidence</th><th>Action</th></tr>{rows}</table></body></html>"
